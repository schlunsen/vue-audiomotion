...
// CONCATENATED MODULE: ./node_modules/cache-loader/dist/cjs.js?{"cacheDirectory":"node_modules/.cache/vue-loader","cacheIdentifier":"1d414548-vue-loader-template"}!./node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!./node_modules/cache-loader/dist/cjs.js??ref--0-0!./node_modules/vue-loader/lib??vue-loader-options!./src/components/AudioMotion.vue?vue&type=template&id=155bc64c&
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{ref:'audioMotion-' + this._uid})}
var staticRenderFns = []


// CONCATENATED MODULE: ./src/components/AudioMotion.vue?vue&type=template&id=155bc64c&
...
// CONCATENATED MODULE: ./src/libs/audioMotion-analyzer.js
...
var audioMotion_analyzer_AudioMotionAnalyzer = /*#__PURE__*/function () {
  /**
   * CONSTRUCTOR
   *
  ...
  function AudioMotionAnalyzer(container) {
    ...
    this._initDone = false; // Settings defaults
    ...
    this._container = container || document.body; // Make sure we have minimal width and height dimensions in case of an inline container

    this._defaultWidth = this._container.clientWidth || 640;
    this._defaultHeight = this._container.clientHeight || 270; // Use audio context provided by user, or create a new one

    var AudioContext = window.AudioContext || window.webkitAudioContext;

    if (options.hasOwnProperty('audioCtx')) {
      if (options.audioCtx instanceof AudioContext) this._audioCtx = options.audioCtx;else throw new audioMotion_analyzer_AudioMotionError('ERR_INVALID_AUDIO_CONTEXT', 'Provided audio context is not valid');
    } else {
      try {
        this._audioCtx = new AudioContext();
      } catch (err) {
        throw new audioMotion_analyzer_AudioMotionError('ERR_AUDIO_CONTEXT_FAIL', 'Could not create audio context. Web Audio API not supported?');
      }
    ...
    this._analyzer.connect(this._audioCtx.destination); // Create canvases
    ...
    this._ledsCtx = this._ledsMask.getContext('2d'); // auxiliary canvas for the X-axis scale labels

    this._labels = document.createElement('canvas');
    this._labelsCtx = this._labels.getContext('2d'); // adjust canvas on window resize

    window.addEventListener('resize', function () {
      if (!_this._width || !_this._height) // fluid width or height
        _this._setCanvas('resize');
    }); // adjust canvas size on fullscreen change

    this._canvas.addEventListener('fullscreenchange', function () {
      return _this._setCanvas('fschange');
    }); // Set configuration options, using defaults for any missing properties


    this._setProperties(options, defaults); // Finish canvas setup
  ...
  _createClass(AudioMotionAnalyzer, [{
    key: "connectAudio",
     ...
     */
    value: function connectAudio(element) {
    ...
    value: function registerGradient(name, options) {
      if (typeof name !== 'string' || name.trim().length == 0) throw new audioMotion_analyzer_AudioMotionError('ERR_GRADIENT_INVALID_NAME', 'Gradient name must be a non-empty string');
      if (_typeof(options) !== 'object') throw new audioMotion_analyzer_AudioMotionError('ERR_GRADIENT_NOT_AN_OBJECT', 'Gradient options must be an object');
      if (options.colorStops === undefined || options.colorStops.length < 2) throw new audioMotion_analyzer_AudioMotionError('ERR_GRADIENT_MISSING_COLOR', 'Gradient must define at least two colors');
    ...
    value: function setFreqRange(min, max) {
      if (min < 1 || max < 1) throw new audioMotion_analyzer_AudioMotionError('ERR_FREQUENCY_TOO_LOW', "Frequency values must be >= 1");else {
    ...
    key: "toggleAnalyzer",
    value: function toggleAnalyzer(value) {
      var _this2 = this;
      ...
      if (started && !value) {
        ...
        this._animationReq = requestAnimationFrame(function (timestamp) {
          return _this2._draw(timestamp);
        });
      }
    ...
    key: "_calculateBarSpacePx",
    value: function _calculateBarSpacePx() {
    ...
    key: "_createLedMask",
    value: function _createLedMask() {
      var _this3 = this;

      // no need for this if in discrete frequencies or area fill modes
      if (this._mode % 10 == 0 || !this._initDone) return;
      var analyzerHeight = this._lumiBars ? this._canvas.height : this._canvas.height * (1 - this._reflexRatio) | 0; // calculates the best attributes for the LEDs effect, based on the visualization mode and canvas resolution
      ...
      var spacing = Math.max(this._ledOptions.spaceH, this._barSpacePx); // clear the auxiliary canvas

      this._ledsMask.width |= 0; // add a vertical black line to the left of each bar to create the LED columns

      this._analyzerBars.forEach(function (bar) {
        return _this3._ledsCtx.fillRect(bar.posX - spacing / 2, 0, spacing, analyzerHeight);
      }); // add a vertical black line in the mask canvas after the last led column


      this._ledsCtx.fillRect(this._analyzerBars[this._analyzerBars.length - 1].posX + this._barWidth - spacing / 2, 0, spacing, analyzerHeight); // add horizontal black lines to create the LED rows


      for (var i = this._ledOptions.ledHeight; i < analyzerHeight; i += this._ledOptions.ledHeight + this._ledOptions.spaceV) {
    ...
    value: function _draw(timestamp) {
      ...
      var isOctaveBands = this._mode % 10 != 0,
          // mode > 0 && mode < 10
      ...
      if (!this.showBgColor) // use black background
        this._canvasCtx.fillStyle = '#000';else if (isLedDisplay) this._canvasCtx.fillStyle = '#111';else this._canvasCtx.fillStyle = this._gradients[this._gradient].bgColor; // use background color defined by gradient
      // clear the canvas

      this._canvasCtx.fillRect(0, 0, this._canvas.width, this._canvas.height); // get a new array of data from the FFT


      this._analyzer.getByteFrequencyData(this._dataArray); // set selected gradient


      this._canvasCtx.fillStyle = this._gradients[this._gradient].gradient; // if in "area fill" mode, start the drawing path
      ...
      var width = this._barWidth - (!isOctaveBands ? 0 : Math.max(isLedDisplay ? this._ledOptions.spaceH : 0, this._barSpacePx)); // if no bar spacing is required, make sure width is integer for pixel accurate calculation

      if (this._barSpace == 0 && !isLedDisplay) width |= 0; // draw bars / lines

      var bar, barHeight;
      var nBars = this._analyzerBars.length;
      ...
      for (var i = 0; i < nBars; i++) {
        ...
        if (bar.endIdx == 0) {
          // single FFT bin
          barHeight = this._dataArray[bar.dataIdx]; // apply smoothing factor when several bars share the same bin

          if (bar.factor) barHeight += (this._dataArray[bar.dataIdx + 1] - barHeight) * bar.factor;
        } else {
          // range of bins
          barHeight = 0; // use the highest value in the range
      ...
      if (this._mode == 10) {
        ...
        this._canvasCtx.drawImage(this._ledsMask, 0, 0); // Reflex effect
      ...
      if (this.showFPS) {
        ...
        this._canvasCtx.font = "bold ".concat(size, "px sans-serif");
    ...
    key: "_findFrequencyBin",
    value: function _findFrequencyBin(freq, func) {
      var bin = freq * this._analyzer.fftSize / this._audioCtx.sampleRate;
    ...
    key: "_findBinFrequency",
    value: function _findBinFrequency(bin) {
    ...
    key: "_generateGradients",
    value: function _generateGradients() {
      var _this5 = this;

      var grad;
      var analyzerHeight = this._lumiBars && this._mode % 10 ? this._canvas.height : this._canvas.height * (1 - this._reflexRatio) | 0;
      Object.keys(this._gradients).forEach(function (key) {
        if (_this5._gradients[key].dir && _this5._gradients[key].dir == 'h') grad = _this5._canvasCtx.createLinearGradient(0, 0, _this5._canvas.width, 0);else grad = _this5._canvasCtx.createLinearGradient(0, 0, 0, analyzerHeight);

        if (_this5._gradients[key].colorStops) {
          _this5._gradients[key].colorStops.forEach(function (colorInfo, index) {
            if (_typeof(colorInfo) == 'object') grad.addColorStop(colorInfo.pos, colorInfo.color);else grad.addColorStop(index / (_this5._gradients[key].colorStops.length - 1), colorInfo);
          });
        }

        _this5._gradients[key].gradient = grad; // save the generated gradient back into the gradients array
      });
    }
    /**
    ...
    key: "_precalculateBarPositions",
    value: function _precalculateBarPositions() {
      var _this6 = this;

      if (!this._initDone) return;
      var minLog, bandWidth;
      this._analyzerBars = [];
      ...
      if (this._mode % 10 == 0) {
        ...
        var minIndex = this._findFrequencyBin(this._minFreq, 'floor');

        var maxIndex = Math.min(this._findFrequencyBin(this._maxFreq), this._analyzer.frequencyBinCount - 1);
        var lastPos = -999;

        for (var i = minIndex; i <= maxIndex; i++) {
          var freq = this._findBinFrequency(i); // frequency represented in this bin


          var pos = Math.round(bandWidth * (Math.log10(freq) - minLog)); // avoid fractionary pixel values
        ...
        var root24 = Math.pow(2, 1 / 24);
        var c0 = 440 * Math.pow(root24, -114); // ~16.35 Hz
        ...
        while ((_freq = c0 * Math.pow(root24, _i)) <= this._maxFreq) {
          if (_freq >= this._minFreq && _i % groupNotes == 0) temperedScale.push(_freq);
          _i++;
        }

        minLog = Math.log10(temperedScale[0]);
        bandWidth = this._canvas.width / (Math.log10(temperedScale[temperedScale.length - 1]) - minLog); // divide canvas space by the number of frequencies (bars) to display
        ...
        temperedScale.forEach(function (freq, index) {
          ...
          var bin = _this6._findFrequencyBin(freq);

          var idx, nextBin; // start from the last used FFT bin

          if (prevBin > 0 && prevBin + 1 <= bin) idx = prevBin + 1;else idx = bin; // FFT does not provide many coefficients for low frequencies, so several bars may end up using the same data
          ...
          if (idx == prevIdx) {
            ...
            if (nBars > 1) {
              for (var _i2 = 1; _i2 <= nBars; _i2++) {
                _this6._analyzerBars[_this6._analyzerBars.length - _i2].factor = (nBars - _i2) / nBars;
              }
            }
          ...
          prevBin = nextBin = bin; // check if there's another band after this one

          if (temperedScale[index + 1] !== undefined) {
            nextBin = _this6._findFrequencyBin(temperedScale[index + 1]); // and use half the bins in between for this band
          ...
          _this6._analyzerBars.push({
            posX: index * _this6._barWidth,
            dataIdx: idx,
            endIdx: endIdx,
      ...
      this._createLedMask(); // Create the X-axis scale in the auxiliary canvas
      ...
      this._labelsCtx.font = "".concat(this._labels.height / 2, "px sans-serif");
      this._labelsCtx.textAlign = 'center';
      var freqLabels = [16, 31, 63, 125, 250, 500, 1000, 2000, 4000, 8000, 16000];

      for (var _i3 = 0, _freqLabels = freqLabels; _i3 < _freqLabels.length; _i3++) {
        var _freq2 = _freqLabels[_i3];

        this._labelsCtx.fillText(_freq2 >= 1000 ? "".concat(_freq2 / 1000, "k") : _freq2, bandWidth * (Math.log10(_freq2) - minLog), this._labels.height * .75);
      }
    }
    /**
    ...
    value: function _setCanvas(reason) {
      ...
      if (this._pixelRatio == 2 && window.screen.height <= 540) this._pixelRatio = 1; // clear the canvas

      this._canvasCtx.fillStyle = '#000';

      this._canvasCtx.fillRect(0, 0, this._canvas.width, this._canvas.height); // set lineJoin property for area fill mode (this is reset whenever the canvas size changes)


      this._canvasCtx.lineJoin = 'bevel'; // (re)generate gradients

      this._generateGradients(); // update LED mask canvas dimensions


      this._ledsMask.width = this._canvas.width;
      this._ledsMask.height = this._canvas.height; // update labels canvas dimensions

      this._labels.width = this._canvas.width;
      this._labels.height = this._pixelRatio * (this.isFullscreen ? 40 : 20); // calculate bar positions and led options

      this._precalculateBarPositions(); // call callback function, if defined
    ...
    key: "_setProperties",
    value: function _setProperties(options, defaults) {
      var callbacks = ['onCanvasDraw', 'onCanvasResize']; // audioCtx is set only at initialization; we handle 'start' after setting all other properties

      var ignore = ['audioCtx', 'start'];
      if (defaults) options = Object.assign(defaults, options);

      for (var _i4 = 0, _Object$keys = Object.keys(options); _i4 < _Object$keys.length; _i4++) {
    ...
    set: function set(value) {
      if (this._gradients.hasOwnProperty(value)) this._gradient = value;else throw new audioMotion_analyzer_AudioMotionError('ERR_UNKNOWN_GRADIENT', "Unknown gradient: '".concat(value, "'"));
    } // Canvas size
    ...
    set: function set(value) {
      ...
      } else throw new audioMotion_analyzer_AudioMotionError('ERR_INVALID_MODE', "Invalid mode: ".concat(mode));
    } // Low-resolution mode
    ...
    set: function set(value) {
      value = Number(value);
      if (value < 0 || value >= 1) throw new audioMotion_analyzer_AudioMotionError('ERR_REFLEX_OUT_OF_RANGE', "Reflex ratio must be >= 0 and < 1");else {
    ...
    set: function set(value) {
      if (value < 1) throw new audioMotion_analyzer_AudioMotionError('ERR_FREQUENCY_TOO_LOW', "Frequency values must be >= 1");else {
        this._minFreq = value;
...
var audioMotion_analyzer_AudioMotionError = /*#__PURE__*/function (_Error) {
  _inherits(AudioMotionError, _Error);

  var _super = _createSuper(AudioMotionError);

  function AudioMotionError(code, message) {
    var _this7;

    _classCallCheck(this, AudioMotionError);

    _this7 = _super.call(this, message);
    _this7.name = 'AudioMotionError';
    _this7.code = code;
    return _this7;
  }
...
/* harmony default export */ var AudioMotionvue_type_script_lang_js_ = ({
  ...
  watch: {
    stream: function stream() {
      ...
      var audioMotion = new audioMotion_analyzer_AudioMotionAnalyzer(this.$refs['audioMotion' + this._uid], this.options);
